...
int main() {
    try {
        ...
        rs2_intrinsics intrinsics = profile.get_stream(RS2_STREAM_COLOR)
                                        ...
                                        .get_intrinsics();
        char root[500];
        double time_length, start_time;
        cout << "please input how many seconds you want to record" << endl;
        cin >> time_length;
        ...
        intrinsic.close();
        createDir(std::string("../data/short_record/"));
        ...
        while (true) {
            ...
            std::ostringstream rgb_filename, depth_filename, filtered_filename;

            // do depth filter

            // cv::Mat filtered_data(cv::Size(width, height), CV_16UC1, (void *)
            // filtered.get_data());

            command = (char)cv::waitKey(25);
            if (command == 27) {
                break;
            } else if (command == 's') {
                command = 0;
                start_time = rgb_frame.get_timestamp();
                snprintf(
                    root, 500, "../data/short_record/%ld/",
                ...
                // createDir(std::string(root) + "filter");
            ...
            // if (is_record)
            // 	cout << rgb_frame.get_timestamp() - start_time <<endl;
            if (is_record && rgb_frame.get_timestamp() - start_time >
                                 time_length * 1e3 + delay) {
                is_record = false;
                rgb_list_TUM.close();
                depth_list_TUM.close();
            }
            ...
            if (!depth_data.empty()) {
                float max_depth = 8;
                cv::Mat bgr, filter_bgr;
                depth_data.convertTo(bgr, CV_8UC1,
                                     (255 / max_depth) * depth_scale);
                bgr = VirdisColorMap(bgr);
                cv::imshow("depth", bgr);

                // filtered_data.convertTo(filter_bgr, CV_8UC1, (255/max_depth)
                // * depth_scale); filter_bgr = VirdisColorMap(filter_bgr);
                // cv::imshow("filter", filter_bgr);
                ...
                if (is_record) {
                    ...
                    // filtered_filename << root << "filter/" <<
                    // std::string(TUM_timestamp_depth) << ".png";
                    // cv::imwrite(filtered_filename.str(), filtered_data);
                }
            }
        }
        pipe.stop();
